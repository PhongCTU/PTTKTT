...
typedef struct
{
    char TenDV[20]; // ten do vat co do dai toi da 20 ky tu
    float TL, GT, DG; // Trong luong, Gia tri, Don gia
    int SL, Phuong_An;    // So phuong an duoc chon
} DoVat;
...
DoVat *Read_File(char file_name[], float *W, int *n)
{
    ...
    while (!feof(f)) // khi con tro con trong file
    {
        ...
        dsdv[i].Phuong_An = 0; //khoi tao phuong an = 0
        i++;
...
void in_dsdv(DoVat *dsdv, int n, float W)
{
    ...
    for (i = 0; i < n; i++)
    {
        printf("|%2d |%-20s|%9.2f|%9.2f|%9.2f|%5d     |%6d     |\n",
               i + 1, dsdv[i].TenDV, dsdv[i].TL, dsdv[i].GT, dsdv[i].DG, dsdv[i].SL,dsdv[i].Phuong_An);
        TongTL = TongTL + dsdv[i].Phuong_An * dsdv[i].TL; // TTL = TTL + so phuong an * Trong luong cua vat
        TongGT = TongGT + dsdv[i].Phuong_An * dsdv[i].GT; // TGT = TGT + so phuong an * Gia tri cu vat
    }
    printf("|---|--------------------|---------|---------|---------|----------|-----------|\n");
...
void greedy(DoVat *dsdv, int n, float W)
{
    ...
    for ( i = 0; i < n; i++)
    {
        ...
        if(dsdv[i].Phuong_An > dsdv[i].SL) dsdv[i].Phuong_An = dsdv[i].SL; // neu so phuong an > so luong cua vat thi so phuong an = so luong vat
